...
22. **GOOGLE REVIEWS LINK — HARD RULE:** Every star rating, review count, and Google badge must link to `https://search.google.com/local/reviews?placeid=[PLACE_ID]`. NEVER link to `google.com/search?q=...#lrd=` (takes visitors to search results, not reviews). If no Place ID is available yet, use Google Maps data URL with `!9m1!1b1` reviews tab parameter as interim, mark config with `place_id_needed: true`, and add HTML comment `TODO: Replace with search.google.com/local/reviews?placeid=`. Never render the rating as a non-clickable `<div>`.
