...
22. **GOOGLE REVIEWS LINK — HARD RULE:** Every star rating, review count, and Google badge must link to `https://search.google.com/local/reviews?placeid=[PLACE_ID]`. NEVER link to `google.com/search?q=...#lrd=` (takes visitors to search results, not reviews). Place ID must be obtained automatically during the Firecrawl/research phase: use Places API if GOOGLE_MAPS_API_KEY is available, or via web search for the `placeid=` URL. Never leave place_id blank in a config. If no Place ID can be found, use Google Maps data URL with `!9m1!1b1` reviews tab parameter as interim, mark config with `place_id_needed: true`. Never render the rating as a non-clickable `<div>`. Never use a Google Maps URL where a Reviews URL is required.
...
40. **CARD GRID SEE MORE:** Any section with more than 4 cards must show only the first 4 by default with a See More toggle button. This applies to services, practice areas, and any other card grid. First 4 are always visible. Remaining cards expand on click. This prevents overwhelming the visitor and keeps pages feeling clean and scannable.
41. **BRAND IDENTITY EXTRACTION — MANDATORY.** After Firecrawl scrapes the client site, scan the full markdown for brand identity signals before writing any code: faith/religious references (bible verses, God, Christ, prayer, ministry), founding story (since, founded, established), signature taglines (repeated phrases, R/TM marks), notable clients (proper nouns near represented/cases against/worked with), personal philosophy (we believe, our approach, our mission), unique positioning (only, first in, exclusively). Every detected signal must be recorded in the config brand_identity field and applied in the build. Faith identity gets a top announcement bar + hero verse + bio mention + trust badge. Taglines appear in the hero subtitle. Notable clients appear in the about section. Missing a visible brand signal that exists on the client's actual site is a build failure equivalent to wrong phone number.
42. **BRAND IDENTITY APPLICATION RULES.** Faith identity: top announcement bar above nav (all devices) + inline verse in hero (desktop, gold, bold, accent colour) + sentence in bio + faith trust badge. Signature tagline: hero subtitle or below headline. Notable clients: about section, trust strip if impressive enough. Personal philosophy: hero subtitle or about section opening. Unique positioning: hero badge or trust strip. The demo must make the business owner feel understood — brand signals are the proof of that understanding.
43. **MEMORY DOCUMENTS:** When creating or updating any system memory file (BRAIN.md, MASTER_BRIEF.md, session logs, capability lists) — always read the source files first. Never write from memory. Read MASTER_BRIEF.md, CLAUDE.md, and the relevant transcript before writing. Memory is unreliable. The files are the source of truth.
44. **MOBILE HERO CONTRAST — CRITICAL:** The hero section on mobile must NEVER have a light or white background with white text. On mobile, if no background image is shown, the hero must use a dark solid background (darkest brand colour or near-black). This is a critical contrast failure if violated. QA script must catch this.
45. **MOBILE NAV VISIBILITY:** The navigation bar on mobile must always have a solid dark background. Transparent navs are only acceptable on desktop when overlaying a dark hero image. On mobile the nav must always be readable — use the darkest brand colour as nav background on mobile.
46. **HERO GRADIENT BALANCE:** The directional gradient on desktop hero images must fade to near-transparent on the right side (rgba(0,0,0,0.15) maximum at 100%). The image must breathe and be visible on the right half of the screen. Never use a gradient that darkens more than 65% across the full width.
47. **HERO HEADLINE SPLIT COLOUR:** Every hero headline must use split colour treatment — wrap 1-2 key words that speak directly to the niche visitor's core desire in var(--primary-light) or var(--accent). This creates visual hierarchy and emotional connection. The split colour word should be the most emotionally resonant word in the headline for that specific niche.
48. **CLIENT LOGO:** If the client has a logo on their existing website or provides one — it must be used in the nav of the demo. Never use text-only nav logo if a real logo exists. Extract via Firecrawl (check img tags near header/nav with keywords: logo, brand, mark). Download and save as logo.svg or logo.png in the client directory. If no logo is found after Firecrawl — text logo is acceptable but must be noted in config as logo_available: false.
49. **MOBILE HERO COLOUR BY NICHE:** The mobile hero background colour is niche-specific. Law = dark navy (white text). Dental = light teal-tinted cream (dark text). Aesthetics/MedSpa = warm cream/blush gradient (dark text). Insurance = light cream with green accent (dark text). Never apply a dark background to an aesthetics site or a light background to a law site. The hero background communicates the brand personality — match it to the niche emotional register.
50. **GOOGLE MAPS EMBED — MANDATORY:** Every local business site must have a maps section with an embedded Google Map and Get Directions button. Use `maps.google.com/maps?q=[encoded_address]&output=embed` — no API key required. Placed between contact/reviews and footer. Directions link: `google.com/maps/dir/?api=1&destination=[encoded_address]`. Background: `var(--cream)`. This is Locked Component 16.
